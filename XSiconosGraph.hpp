...
#ifndef XSICONOS_GRAPH_HPP
#define XSICONOS_GRAPH_HPP
...
public:
  ...
  */
  ...
  friend void siconos_io(Archive&, XSiconosGraph < V, E, VProperties, EProperties,
                         GProperties > &,
                         const unsigned int);
...
private:
  ...
  XSiconosGraph(const XSiconosGraph&);

public:

  /** default constructor
   */
  XSiconosGraph() : _stamp(0)
  {
  };
...
//  inline auto& index(const VDescriptor& vd)
//  {
...
//      std::cout << ":" << index(new_vertex_descriptor) <<std::endl;
  ...
  void remove_vertex(const V& vertex_bundle)
  {
    ...
    for (std::tie(vi, viend) = vertices(); vi != viend; ++vi)
    {
      vertex_descriptor[bundle(*vi)] = index(*vi);
    }
