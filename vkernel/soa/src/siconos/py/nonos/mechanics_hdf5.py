...
def topods_shape_reader(shape, deflection=0.001):
    ...
    with tmpfile(suffix='.stl', debug=True) as tmpf:
        ...
        error = stl_writer.Write(mesh.Shape(), tmpf[1])
        tmpf[0].flush()

        if (not error and os.path.getsize(tmpf[1]) > 0):
...
def brep_reader(brep_string, indx):
    ...
    with tmpfile(suffix='.stl') as tmpf:
        error = stl_writer.Write(shape, tmpf[1])
        tmpf[0].flush()
