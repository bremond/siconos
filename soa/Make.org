...
#+begin_src sh :compile :results output silent
  guix time-machine -C ${src_dir}/siconos-2023-13-12.scm -- \
       shell --container -N --emulate-fhs --expose=${src_dir} --with-input=gcc-toolchain=gcc-toolchain@12 siconos benchmark gcc-toolchain suitesparse eigen boost fmt range-v3 cmake make dash python pybind11 -- \
...
#+begin_src sh :compile :results output silent
  guix time-machine -C ${src_dir}/siconos-2023-13-12.scm -- \
       shell --container -N --emulate-fhs --expose=${src_dir} siconos benchmark gcc-toolchain suitesparse eigen boost fmt range-v3 cmake make dash python pybind11 -- \
       make VERBOSE=1
#+end_src
...
#+name: mybenchmarks-make-disks-release-clang
#+header: :var src_dir=src-dir
#+header: :dir build-clang-release
#+begin_src sh :compile :results output silent
  guix time-machine -C ${src_dir}/siconos-2023-11-08.scm -- \
       shell --container -N --emulate-fhs --expose=${src_dir} siconos benchmark clang-toolchain suitesparse eigen boost fmt range-v3 cmake make dash python pybind11 -- \
       make VERBOSE=1 disks && { cd src/siconos/tests ; ./disks ; }
#+end_src
...
#+begin_src sh :compile :results output silent
  guix time-machine -C ${src_dir}/siconos-2023-11-08.scm -- \
       environment siconos --pure --ad-hoc siconos benchmark clang-toolchain suitesparse eigen boost fmt range-v3 cmake make dash python pybind11 -- \
       make VERBOSE=1
#+end_src

#+name: mybenchmarks-make-disks-debug-clang
#+header: :var src_dir=src-dir
#+header: :dir build-clang-debug
#+begin_src sh :compile :results output silent
  guix time-machine -C ${src_dir}/siconos-2023-11-08.scm -- \
