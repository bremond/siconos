...
// Default
ZSiconosVector::ZSiconosVector()
{
  _dense = true;
  vect.Dense = new ZDenseVect(ublas::zero_vector<double>());
}

// parameters: dimension and type.
ZSiconosVector::ZSiconosVector(unsigned row, Siconos::UBLAS_TYPE type)
{
  ...
  {
    _dense = true;
    vect.Dense = new ZDenseVect(ublas::zero_vector<double>(row));
  }
  else
  {
...
ZSiconosVector::ZSiconosVector(unsigned row, double val, Siconos::UBLAS_TYPE type)
{
  if(type == Siconos::SPARSE)
  {
    _dense = false;
  ...
  {
    _dense = true;
    vect.Dense = new ZDenseVect(ublas::scalar_vector<double>(row, val));
  }
  else
  {
    THROW_EXCEPTION("invalid type");
...
ZSiconosVector::ZSiconosVector(const std::vector<double>& v, Siconos::UBLAS_TYPE typ)
{
  if(typ != Siconos::DENSE)
    THROW_EXCEPTION("invalid type");

  _dense = true;
  vect.Dense = new ZDenseVect(v.size());
  std::copy(v.begin(), v.end(), (vect.Dense)->begin());
}

// Copy
ZSiconosVector::ZSiconosVector(const ZSiconosVector &svect) : std::enable_shared_from_this<ZSiconosVector>()
{
  if(true)  // dense
  {
    _dense = true;
    vect.Dense = new ZDenseVect(svect.size());
...
ZSiconosVector::ZSiconosVector(const ZDenseVect& m)
{
  _dense = true;
  vect.Dense = new ZDenseVect(m.size());
  noalias(*vect.Dense) = m;

}

ZSiconosVector::ZSiconosVector(const SparseVect& m)
{
  _dense = false;
...
ZSiconosVector::ZSiconosVector(const std::string &file, bool ascii)
{
}

ZSiconosVector::ZSiconosVector(const ZSiconosVector& v1, const ZSiconosVector& v2)
{
}

// Copy a block vector into a ZSiconosVector
// This is mostly used to handle contiguous memory.
ZSiconosVector::ZSiconosVector(const BlockVector & input)
{
...
SparseVect* ZSiconosVector::sparse()const
{
...
double* ZSiconosVector::getArray() const
{
  assert(vect.Dense && "ZSiconosVector::getArray() : not yet implemented for sparse vector.");
...
void ZSiconosVector::fill(double value)
{
  if(!_dense)
  {
...
void ZSiconosVector::resize(unsigned int n, bool preserve)
{
  if(_dense)
...
double ZSiconosVector::normInf() const
{
  if(_dense)
...
double ZSiconosVector::norm2() const
{
  if(_dense)
...
//=====================================
double ZSiconosVector::vector_sum() const
{
  if(_dense)
...
void ZSiconosVector::display()const
{
...
std::string ZSiconosVector::toString() const
{
...
std::ostream& operator<<(std::ostream& os, const ZSiconosVector& sv)
{
  if(sv._dense)
    os << *sv.vect.Dense;
  else
...
double ZSiconosVector::getValue(unsigned int row) const
{
  assert(row < size() && "ZSiconosVector::getValue(index) : Index out of range");
...
void ZSiconosVector::setValue(unsigned int row, double value)
{
  assert(row < size() && "ZSiconosVector::setValue(index, value) : Index out of range");
  if(_dense)
...
double& ZSiconosVector::operator()(unsigned int row)
{
  assert(row < size() && "ZSiconosVector::operator ( index ): Index out of range");
...
double ZSiconosVector::operator()(unsigned int row) const
{
  assert(row < size() && "ZSiconosVector::operator ( index ): Index out of range");
...
void ZSiconosVector::setBlock(unsigned int index, const ZSiconosVector& vIn)
{
  // Set current vector elements, starting from position "index", to the values of vector vIn

  // Exceptions ...
  assert(&vIn != this && "ZSiconosVector::this->setBlock(pos,vIn): vIn = this.");

  assert(index < size() && "ZSiconosVector::setBlock : invalid ranges");

  auto end = vIn.size() + index;
  assert(end <= size() && "ZSiconosVector::setBlock : invalid ranges");

  assert(vIn.num() == num() && "ZSiconosVector::setBlock: inconsistent types.");
...
void ZSiconosVector::toBlock(ZSiconosVector& vOut, unsigned int sizeB, unsigned int startIn, unsigned int startOut) const
{
...
void ZSiconosVector::addBlock(unsigned int index, const ZSiconosVector& vIn)
{
...
void ZSiconosVector::subBlock(unsigned int index, const ZSiconosVector& vIn)
{
...
ZSiconosVector& ZSiconosVector::operator = (const ZSiconosVector& vIn)
{
  if(&vIn == this) return *this;  // auto-assignment.

  assert(size() == vIn.size() && "ZSiconosVector::operator = failed: inconsistent sizes.");
...
ZSiconosVector& ZSiconosVector::operator = (const BlockVector& vIn)
{
}


ZSiconosVector& ZSiconosVector::operator = (const ZDenseVect& d)
{
  if(!_dense)
...
ZSiconosVector& ZSiconosVector::operator = (const SparseVect& sp)
{
  if(_dense)
...
ZSiconosVector& ZSiconosVector::operator = (const double* d)
{
  assert(_dense && "ZSiconosVector::operator = double* : forbidden: the current vector is not dense.");

  siconosBindings::detail::copy(vect.Dense->size(), d, 1, getArray(), 1);
  return *this;
}

unsigned ZSiconosVector::copyData(double* data) const
{
  assert(_dense && "ZSiconosVector::copyData : forbidden: the current vector is not dense.");
...
bool operator == (const ZSiconosVector &m, const ZSiconosVector &x)
{
...
ZSiconosVector operator * (const  ZSiconosVector&m, double d)
{
...
ZSiconosVector operator * (double d, const  ZSiconosVector&m)
{
  Siconos::UBLAS_TYPE numM = m.num();
...
ZSiconosVector operator + (const  ZSiconosVector& x, const  ZSiconosVector& y)
{
  if(x.size() != y.size())
  ...
  if(numX == numY)  // x, y ZSiconosVector of the same type
  {
    if(numX == Siconos::DENSE)
    {
      //    siconosBindings::xpy(*x.dense(),p);
      //    return p;
      return (ZDenseVect)(*x.dense() + *y.dense());
    }
    else
      return (SparseVect)(*x.sparse() + *y.sparse());
  }

  else // x, y ZSiconosVector with y and x of different types
  {
    if(numX == Siconos::DENSE)
      return (ZDenseVect)(*x.dense() + *y.sparse());
    else
      return (ZDenseVect)(*x.sparse() + *y.dense());
  }

}

void add(const ZSiconosVector& x, const ZSiconosVector& y, ZSiconosVector& z)
{
  ...
  else // No common memory between x,y and z
  {
    ...
    if(numZ != 0)  // z is a ZSiconosVector
    {
      if(numX == numY && numX != 0)  // x, y ZSiconosVector of the same type
      {
        if(numX == Siconos::DENSE)
        {
...
void sub(const ZSiconosVector& x, const ZSiconosVector& y, ZSiconosVector& z)
{
  // Computes z = x - y in an "optimized" way (in comparison with operator +)
...
void axpby(double a, const ZSiconosVector& x, double b, ZSiconosVector& y)
{
  // Computes y = ax + by
...
void axpy(double a, const ZSiconosVector& x, ZSiconosVector& y)
{
  // Computes y = ax + y
...
double inner_prod(const ZSiconosVector &x, const ZSiconosVector &m)
{
  if(x.size() != m.size())
    THROW_EXCEPTION("inconsistent sizes");
...
SimpleMatrix outer_prod(const ZSiconosVector &x, const ZSiconosVector& m)
{
  Siconos::UBLAS_TYPE numM = m.num();
  Siconos::UBLAS_TYPE numX = x.num();
...
void scal(double a, const ZSiconosVector & x, ZSiconosVector & y, bool init)
{
...
void subscal(double a, const ZSiconosVector & x, ZSiconosVector & y, const Index& coord, bool init)
{
  ...
  if(&x == &y)  // if x and y are the same object
  {
    ...
    {
      ublas::vector_range<ZDenseVect> subY(*y.dense(), ublas::range(coord[2], coord[3]));
      if(coord[0] == coord[2])
      {
        if(init)
          subY *= a;
      ...
      {
        ublas::vector_range<ZDenseVect> subX(*x.dense(), ublas::range(coord[0], coord[1]));
        if(init)
          subY = a * subX;
        else
          subY += a * subX;
...
void cross_product(const ZSiconosVector& V1, const ZSiconosVector& V2, ZSiconosVector& VOUT)
{
...
void abs_wise(const ZSiconosVector& V, ZSiconosVector& Vabs)
{
...
void getMax(const ZSiconosVector& V, double& maxvalue, unsigned int& idmax)
{
...
void getMin(const ZSiconosVector& V, double& minvalue, unsigned int& idmin)
{
...
void setBlock(const ZSiconosVector& vIn, SP::ZSiconosVector vOut, unsigned int sizeB,
...
unsigned int ZSiconosVector::size(void) const
{
  if(!_dense)
  {
...
ZSiconosVector& operator *= (ZSiconosVector& v, const double& s)
{
  if(v._dense)
    *v.dense() *= s;
  else
...
ZSiconosVector::iterator ZSiconosVector::begin()
{
//  return ZSiconosVector::iterator(*this, 0);
}

ZSiconosVector::const_iterator ZSiconosVector::begin() const
{
}

ZSiconosVector::iterator ZSiconosVector::end()
{
}

ZSiconosVector::const_iterator ZSiconosVector::end() const
{
}

ZSiconosVector::operator std::vector<double>()
{
